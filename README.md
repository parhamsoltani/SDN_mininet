# SDN_mininet
 This is a repository for SDN-based implementation of MPLS network with OVS and OpenFlow and Network Virtualization using OpenDayLight controller (version: Nitrogen) for remote approaches.

https://docs.openvswitch.org/en/latest/contents/

http://mininet.org/walkthrough/

https://github.com/mininet/openflow

https://networkx.org/

https://docs.opendaylight.org/en/stable-sulfur/documentation.html

https://nexus.opendaylight.org/content/repositories/opendaylight.release/org/opendaylight/integration/karaf/0.7.3/
